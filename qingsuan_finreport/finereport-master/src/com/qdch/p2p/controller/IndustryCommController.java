...
public class IndustryCommController extends BaseController {
	...
	public void index() {
		...
		/*
		 * setAttr("companyName",
		 * PpjyscModel.dao.getJyscGs(getDataScopeByUserNameForP2p()));
		 */

		List<CompanysInfoModel> info = CompanysInfoModel.dao.getInfo(
				getDataScopeByUserNameForP2p(), name);
		setAttr("companyName", info);

		List<CompanysInfoModel> basic = CompanysInfoModel.dao.getBasicinfo(
				getDataScopeByUserName(), name);
		setAttr("basic", basic);
		List<CoShareHolderModel> main = CoShareHolderModel.dao
				.getMainperson(name);
		setAttr("main", main);
		List<CoShareHolderModel> manager = CoShareHolderModel.dao
				.getManager(name);
		setAttr("manager", manager);
		List<CoShareHolderModel> dierctor = CoShareHolderModel.dao
				.getDong(name);
		setAttr("director", dierctor);
		List<CoShareHolderModel> supervisor = CoShareHolderModel.dao
				.getJian(name);
		setAttr("supervisor", supervisor);
		setAttr("core", CoShareHolderModel.dao.getMainSize(name).size()
				+ CoShareHolderModel.dao.getManagerSize(name).size()
				+ CoShareHolderModel.dao.getDirectSize(name).size()
				+ CoShareHolderModel.dao.getSuperSize(name).size());
		List<CoShareHolderModel> stackInfo = CoShareHolderModel.dao
				.getStackInfo(name);
		setAttr("stackInfo", stackInfo);
		setAttr("stacksize", stackInfo.size());
		List<CoShareHolderModel> invest = CoShareHolderModel.dao
				.getInvest(name);
		...
		List<CoChangeLogModel> nameChange = CoChangeLogModel.dao
				.getNamechange(name);
		setAttr("nameChange", nameChange);
		List<CoChangeLogModel> investChange = CoChangeLogModel.dao
				.getInvestchange(name);
		setAttr("investChange", investChange);
		List<CoChangeLogModel> liaison = CoChangeLogModel.dao
				.getLiaisonchange(name);
		setAttr("liaison", liaison);
		setAttr("liaiSize", liaison.size());
		List<CoChangeLogModel> business = CoChangeLogModel.dao
				.getBuinesschange(name);
		setAttr("business", business);
		List<CoChangeLogModel> companyType = CoChangeLogModel.dao
				.getComtypechange(name);
		setAttr("com", companyType);
		setAttr("typesize", companyType.size());
		List<CoChangeLogModel> register = CoChangeLogModel.dao
				.getRegisterchange(name);
		...
		CoDishonestyModel dishonesty = CoDishonestyModel.dao
				.getDishonesty(name);
		...
		List<CoBusinessModel> businesExeption = CoBusinessModel.dao
				.getBusiness(name);
		...
		setAttr("count", count);
		List<CoShareHolderModel> stockC = CoShareHolderModel.dao
				.getStockc(name);
		setAttr("stockc", stockC);
		CoEnterpriseModel cc = CoEnterpriseModel.dao.getid(name);
		if (cc == null)
			cc = new CoEnterpriseModel();
		...
		setAttr("all",
				CoShareHolderModel.dao.getMainSize(name).size()
						+ CoShareHolderModel.dao.getManagerSize(name).size()
						+ CoShareHolderModel.dao.getDirectSize(name).size()
						+ CoShareHolderModel.dao.getSuperSize(name).size()
						+ stackInfo.size() + invest.size() + changeLog.size()
						+ branch.size() + liaison.size() + register.size()
						+ companyType.size());
		setAttr("allsize", judgment.size() + announce.size() + exector.size()
				+ businesExeption.size() + penaltv.size() + job.size());
		List<CoMainPersonModel> senior = new ArrayList<CoMainPersonModel>();
		if (basic != null && basic.size() > 0) {
			senior = CoMainPersonModel.dao.getRelact(basic.get(0).get(
					"legal_person")
					+ "");
		}
	...
	public void gainCore() {
		String name = getPara("name");
		List<CoCorePersonModel> corePerson = CoCorePersonModel.dao
				.getMainperson(name);
	...
	public void gainBrach() {
		String name = getPara("name");
		List<CoCompanyBrachModel> brachs = CoCompanyBrachModel.dao
				.getBrach(name);
		mRenderJson(brachs);
	}
	...
	public void gainExecption() {
		String name = getPara("name");
		List<CoExecptionModel> execption = CoExecptionModel.dao
				.getExecption(name);
	...
	public void gainShareholder() {
		String name = getPara("name");
		List<CoShareholderInfoModel> shareholder = CoShareholderInfoModel.dao
				.getShareholder(name);
	...
	public void gainBusinesstype() {
		String name = getPara("name");
		CoBusinessTypeModel business = CoBusinessTypeModel.dao
